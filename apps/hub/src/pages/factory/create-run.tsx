/**
 * Factory Create Run — wizard de 2 passos com inferência de specs.
 * Portado de sneak-peek-hub/src/panels/CreateRunPanel.tsx
 */

import { useState, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import { inferFromSpecs, createRun } from '@/lib/factory-api';
import { Button } from '@/components/ui/button';
import { cn } from '@/lib/utils';

type Step = 'infer' | 'confirm';

export function FactoryCreateRunPage() {
  ...
  const handleAdvance = useCallback(async () => {
    setInferring(true);
    try {
      const result = await inferFromSpecs(specsPath.trim());
      setSlug(result.suggestedSlug || '');
      setName(result.suggestedName || '');
      setWorkspace(result.workspace || '');
      setSpecs(result.specs || specsPath.trim());
    } catch {
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      navigate(`/factory/runs/${result.slug}/manage`);
    } catch (e: any) {
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-md border bg-background text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary"
              />
            </div>
            <Button onClick={handleAdvance} disabled={!specsPath.trim() || inferring}>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-3">
              ...
              <div className="pt-2 border-t">
                ...
                <div className="grid grid-cols-1 gap-3">
                  <LimitField label="Turns por sessão" hint="Quanto o agente pode trabalhar em cada feature" value={maxTurns} onChange={setMaxTurns} unlimited={maxTurnsUnlimited} onToggleUnlimited={setMaxTurnsUnlimited} placeholder="200" />
                  <LimitField label="Iterações do loop" hint="Quantas vezes o loop roda (features + retries)" value={maxIterations} onChange={setMaxIterations} unlimited={maxIterationsUnlimited} onToggleUnlimited={setMaxIterationsUnlimited} placeholder="50" />
                  <LimitField label="Max features" hint="Quantas features completar antes de parar" value={maxFeatures} onChange={setMaxFeatures} unlimited={maxFeaturesUnlimited} onToggleUnlimited={setMaxFeaturesUnlimited} placeholder="10" />
                </div>
              </div>
            </div>
            {createError && (
            ...
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setStep('infer')}>Voltar</Button>
              <Button onClick={handleCreate} disabled={!slug.trim() || !workspace.trim() || !specs.trim() || creating}>
  ...
  return (
    <div>
      ...
      <input
        ...
        className={cn(
          'w-full px-3 py-2 rounded-md border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary',
          mono && 'font-mono',
        )}
      />
    </div>
