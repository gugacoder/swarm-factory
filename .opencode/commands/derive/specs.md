---
description: Deriva especificações estruturadas a partir do brainstorming de uma sessão de planning
---
...
```
.planning/.what-is/SPECS.md          ← Formato canônico (OSD, US, ER, Design, UI, Feature)
.planning/.template/02-specs/*.md    ← Templates dos arquivos de saída
```

Entenda e memorize TODOS os padrões antes de prosseguir. Esses padrões são VINCULANTES.

### 2. Identifique a sessão de planning

O argumento passado é o nome da sessão (ex: `01-mvp-cartao-de-ponto`).

Caminhos:
- **Input**: `.planning/{sessão}/01-brainstorming/` — TODOS os arquivos .md
- **Output**: `.planning/{sessão}/02-specs/` — arquivos de spec
- **Refs**: `.planning/{sessão}/04-refs/` — referências técnicas adicionais

### 3. Leia TODOS os brainstormings

Leia cada arquivo em `.planning/{sessão}/01-brainstorming/` **na íntegra**. Para arquivos grandes (>500 linhas), leia em chunks de 500 linhas. Não pule nenhum arquivo. Confirme a leitura de cada um antes de prosseguir.
...
Para cada arquivo, leia o template em `.planning/.template/02-specs/` e o stub existente em `.planning/{sessão}/02-specs/` antes de escrever.
